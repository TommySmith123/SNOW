...
test("keeps core game contracts explicit and configurable", async () => {
  ...
  assert.match(game, /headWidthScale/);
  assert.match(game, /One perspective transform drives the jacket, head, hair and equipment/);
  assert.match(game, /ctx\.transform\(bodyWidthScale, depthShear, 0, 1, 0, 0\)/);
  assert.match(game, /createLinearGradient\(-14, 0, 14, 0\)/);
  assert.match(game, /function drawRiderArm/);
  assert.match(game, /resolveCarveUpperBodyProjection/);
  assert.match(game, /headScaleCorrection/);
  assert.match(game, /A crash rotates the already assembled carve pose/);
  assert.match(game, /const carving = ACTIVE_TURN_STYLE === "carve"/);
  ...
  assert.match(turning, /bodyYaw = \(1 - turnBlend\) \* \(Math\.PI \/ 2\)/);
  assert.match(turning, /bodyWidthScale: 0\.62 \+ facingAmount \* 0\.38/);
  assert.match(turning, /headWidthScale: 0\.7 \+ facingAmount \* 0\.3/);
  assert.match(turning, /depthShear: sideAmount \* 0\.18/);
  assert.match(turning, /function resolveCarveUpperBodyProjection/);
